let arreglo = ["", "", "", "", ""];

function allowDrop(ev) {
    ev.preventDefault();
}

function drag(ev) {
    ev.dataTransfer.setData("text", ev.target.id);
}

function drop(ev) {
    if (arreglo[parseInt(ev.target.id)] === "") {
        var data = ev.dataTransfer.getData("text");
        arreglo[parseInt(ev.target.id)] = data;
        ev.target.appendChild(document.getElementById(data));
    }
    
    // Controlamos que todos los elementos estén arrastrados
    if (arreglo[0] !== "" && arreglo[1] !== "" && arreglo[2] !== "" && arreglo[3] !== "" && arreglo[4] !== "") {
        // Controlamos si hay coincidencia
        if (arreglo[0] === "f" && arreglo[1] === "c" || "g" && arreglo[2] === "d" && arreglo[3] === "g" || "c" && arreglo[4] === "b") {
            document.querySelector("h2").innerHTML = "¡Excelente!";
        } else {
            document.querySelector("h2").innerHTML = "¡Qué mal, inténtalo de nuevo!";
        }
    }
}
